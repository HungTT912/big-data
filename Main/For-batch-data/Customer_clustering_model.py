...
def kmeans_clustering(scaled_customer_seg):
    ...
    Output: Tuple of Silhouette score, cluster distribution counts, and predictions DataFrame
    '''
    # Train KMeans model
    ...
    return silhouette, cluster_distribution_counts, predictions



def print_clusters(predictions, cluster_distribution_counts):
    ''' 
    Description: Prints information related to each cluster, including samples and descriptive statistics.
    Input: Predictions DataFrame (predictions) and cluster_distribution_counts
...
def main():
    ...
    customer_data = spark.read.csv("hdfs://namenode:9000/data/E-commerceCustomerBehavior-Sheet1.csv", header=True, inferSchema=True)

    # Preprocessing
    scaled_customer_seg = preprocessing(customer_data)
    
    # KMeans clustering
    silhouette, cluster_distribution_counts, predictions = kmeans_clustering(scaled_customer_seg)
    
    # Print clusters
    print_clusters(predictions, cluster_distribution_counts)
    scaled_customer_seg.explain()
